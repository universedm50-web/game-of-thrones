...
// ============================================================
// 1. CHARACTER DATABASE — FULL WITH RELIABLE IMAGE URLs
// ============================================================
const characters = [
    // ─── HOUSE STARK ───────────────────────────────────────────
    {
        ...
        name: "Jon Snow",
        house: "Stark",
        role: "Vua Phương Bắc & Người thừa kế Ngai Sắt",
    ...
    },
    {
        ...
        milestones: [
            ...
            { title: "Tiêu diệt Dạ Vương cứu thế giới (Phần 8)", desc: "Trực tiếp đâm chết Dạ Vương trong Đêm Trường đại chiến, giải cứu nhân loại khỏi diệt vong." }
        ]
    },
    {
        id: "robb_stark",
        ...
        milestones: [
            ...
            { title: "Bi thảm Tiệc Cưới Đỏ (Phần 3)", desc: "Phá vỡ giao ước kết hôn với nhà Frey. Bị nhà Frey và Bolton âm mưu phản bội sát hại dã man ngay tại tiệc cưới." }
        ]
    },
    {
        id: "bran_stark",
        ...
        bio: "Bran bị phế liệt hai chân từ nhỏ do bị Jaime Lannister đẩy ngã. Tuy nhiên, anh sở hữu năng lực tâm linh vượt trội giúp anh đi sâu vào quá khứ, tương lai và trở thành Quạ Ba Mắt kế nhiệm.",
        milestones: [
    ...
    },
    {
        ...
        sigil: "assets/images/sigil_lannister.jpg",
        status: "Đã mất",
        weapon: "Kiếm vàng hoàng gia và kiếm Oathkeeper",
        motto: "Hear Me Roar!",
        ...
        milestones: [
            ...
            { title: "Lưu lạc và mất bàn tay kiếm (Phần 3)", desc: "Bị Locke chặt đứt bàn tay phải làm kiếm sĩ, trải qua hành trình sinh tử cùng Brienne xứ Tarth." },
    ...
    },
    {
        ...
        milestones: [
            ...
            { title: "Bão Hoang Hỏa quét sạch kẻ thù (Phần 6)", desc: "Dùng thuốc nổ thiêu rụi toàn bộ Đại điện Baelor, xóa sổ Giáo phái Thất diện thần và gia tộc Tyrell." },
    ...
    },
    {
        ...
        bio: "Con trai cả của Vua Điên Aerys và là cha ruột của Jon Snow. Khác với người cha điên cuồng, Rhaegar là hiệp sĩ hào hoa, tài năng và được nhân dân vô cùng kính trọng.",
        milestones: [
    ...
    },
    {
        ...
        milestones: [
            ...
            { title: "Tế sống con gái Shireen & Kết cục (Phần 5)", desc: "Bị Melisandre thuyết phục thiêu sống con gái ruột tế thần để tan tuyết. Bị quân Bolton tiêu diệt tại Winterfell." }
        ]
    },
    {
        id: "renly_baratheon",
...
// ============================================================
// 2. DRAGONS DATABASE — FULL WITH RELIABLE IMAGE URLs
// ============================================================
const dragons = [
    {
        id: "balerion",
    ...
    },
    {
        ...
        size: "Cực kỳ khổng lồ (Lớn nhất thời kỳ Dance of the Dragons)",
    ...
    },
    {
        ...
        image: "https://images.unsplash.com/photo-1567095761054-7a02e69e5c43?w=600&h=400&fit=crop&q=80",
        category: "beautiful",
    ...
    },
    {
        ...
        power: "Tốc độ bay vô địch, móng vuốt bén có sức xé xác cực cao",
        strengths: "Con rồng nhanh nhất thế giới thời điểm đó, cực kỳ can đảm",
        image: "https://images.unsplash.com/photo-1560707854-fb9a95e65b01?w=600&h=400&fit=crop&q=80",
        category: "combat",
...
const quotes = [
    ...
    { text: "The night is dark and full of terrors.", author: "Phù thủy Đỏ Melisandre" }
];
...
class ParticleSystem {
    ...
    draw() {
        ...
        for (let p of this.particles) {
            ...
            this.ctx.fillStyle = `rgba(201, 160, 84, ${p.opacity * 0.35})`;
            this.ctx.fill();
        }
    }
    animate() {
...
// ============================================================
// 5. SPA NAVIGATION ROUTER — FIXED (click + hashchange)
// ============================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ── Intercept ALL page-nav-link clicks (nav, buttons, home feature links)
    ...
    // ── Also listen to browser back/forward hashchange
    ...
    // ── Dragon filter buttons
    document.querySelectorAll('.dragon-filter-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            document.querySelectorAll('.dragon-filter-btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
    ...
    const revealElements = () => {
        document.querySelectorAll('.reveal').forEach(rev => {
            if (rev.getBoundingClientRect().top < window.innerHeight - 120) {
...
function getHouseColor(house) {
    return { Stark: '#5b8dd9', Lannister: '#c9a054', Targaryen: '#c0392b', Baratheon: '#d4af37' }[house] || '#c9a054';
}

function makePlaceholderSVG(label, color) {
    const initial = (label || '?').replace(/[<>&"]/g, '').charAt(0).toUpperCase();
    const safeName = (label || '').replace(/[<>&"]/g, '').substring(0, 22).toUpperCase();
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="400" height="500" viewBox="0 0 400 500">
      ...
      <rect x="1" y="1" width="398" height="498" fill="none" stroke="${color}" stroke-width="1.5" opacity="0.3"/>
      <text x="200" y="240" font-size="96" text-anchor="middle" dominant-baseline="middle" fill="${color}" opacity="0.55" font-family="Georgia,serif">${initial}</text>
      <text x="200" y="340" font-size="13" text-anchor="middle" fill="${color}" opacity="0.35" font-family="Arial,sans-serif" letter-spacing="3">${safeName}</text>
    </svg>`;
...
function initQuoteSlider() {
    ...
    quotes.forEach((q, idx) => {
        ...
        item.innerHTML = `<p class="quote-text">"${q.text}"</p><span class="quote-author">— ${q.author}</span>`;
        ...
        card.innerHTML = `
            <div class="char-img-container">
                ...
                <div class="char-sigil-badge" title="Gia Huy Nhà ${c.house}">
                    <img src="${c.sigil}" alt="${c.house} sigil" onerror="this.style.display='none'">
                </div>
            </div>
            <div class="char-info">
                ...
                <button class="char-action-btn" data-char-id="${c.id}">
                    <span>Xem Hành Trình</span>
                    <i class="fa-solid fa-crown"></i>
                </button>
            </div>`;
        card.querySelector('.char-action-btn').addEventListener('click', () => openDetails(c.id));
    ...
    list.forEach(d => {
        ...
        card.innerHTML = `
            ...
            <div class="char-info">
                ...
                <button class="char-action-btn" data-dragon-id="${d.id}">
                    <span>Xem Sức Mạnh</span>
                    <i class="fa-solid fa-fire-burner"></i>
                </button>
            </div>`;
        card.querySelector('.char-action-btn').addEventListener('click', () => openDragonDetails(d.id));
        grid.appendChild(card);
    });
}
...
function openDetails(charId) {
    ...
    const fallbackSrc = makePlaceholderSVG(char.name, houseColor);
    const milestonesHTML = (char.milestones || []).map(m => `
    ...
    document.getElementById('modal-body-content').innerHTML = `
        <div class="char-details-layout">
            ...
            <div class="modal-info-panel">
                <span class="modal-house-title" style="color:${houseColor}">Gia tộc ${char.house}</span>
                ...
                ${milestonesHTML ? `<h4 class="modal-subtitle">Các ải hành trình trong phim</h4>
                <div class="modal-milestones"><div class="milestone-list">${milestonesHTML}</div></div>` : ''}
                ...
                <div class="modal-attributes">
                    ...
                    <div class="attribute-item">
                        ...
                        <div class="attribute-value" style="font-style:italic;color:var(--gold-primary)">"${char.motto}"</div>
                    </div>
                </div>
            </div>
        </div>`;
...
function openDragonDetails(dragonId) {
    ...
    if (!dragon) return;
    const catColors = { huge: '#8b4dbb', combat: '#c0392b', beautiful: '#c9a054' };
    const color     = catColors[dragon.category] || '#c9a054';
    const fallbackSrc = makePlaceholderSVG('🐉 ' + dragon.name, color);
    const milestonesHTML = (dragon.milestones || []).map(m => `
        <div class="milestone-item">
            <div class="milestone-title">${m.title}</div>
            <div class="milestone-desc">${m.desc}</div>
        </div>`).join('');
    ...
    document.getElementById('modal-body-content').innerHTML = `
        <div class="char-details-layout">
            ...
            <div class="modal-info-panel">
                <span class="modal-house-title" style="color:${color}">Rồng Thần Targaryen</span>
                ...
                ${milestonesHTML ? `<h4 class="modal-subtitle">Các chiến tích lịch sử</h4>
                <div class="modal-milestones"><div class="milestone-list">${milestonesHTML}</div></div>` : ''}
